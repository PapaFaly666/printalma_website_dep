...
export interface Transformation {
  ...
  positionId?: string; // 🆕 Ajout du positionId selon la documentation
}
...
// 🆕 Types d'erreur selon la documentation
...
// =====================================================================
// Helpers (copiés depuis vendorProductService pour éviter import circulaire)
// =====================================================================
...
// 🆕 Gestion des erreurs selon la documentation - Étape 6
...
class TransformationService {
  ...
  /**
   * 🆕 Créer une transformation (prototype) via POST /vendor/products
   * Selon la documentation - Étape 3
   */
  async createTransformation(payload: {
    ...
    try {
      ...
      if (result.status !== 'TRANSFORMATION') {
        throw new TransformationError(500, 'Réponse inattendue du serveur', { result });
      }
      ...
      throw new TransformationError(500, 'Erreur lors de la création de la transformation', error);
    }
  }

  /**
   * Liste des transformations (prototypes) - Étape 4
  ...
  async getTransformations(params?: {
    ...
    try {
      ...
      throw new TransformationError(500, 'Erreur lors de la récupération des transformations', error);
    }
  }

  /**
   * Publier (convertir) une transformation en produit réel - Étape 5
  ...
  /**
   * Nettoyer les transformations (optionnel) - Étape 7
  ...
  /**
   * 🆕 Supprimer une transformation spécifique
  ...
  /**
   * 🆕 Obtenir les détails d'une transformation
  ...
  async getTransformation(id: number): Promise<Transformation> {
    try {
      ...
      throw new TransformationError(500, 'Erreur lors de la récupération de la transformation', error);
    }
  }

  /**
   * 🆕 Utilitaire pour vérifier si une transformation peut être publiée
   */
  ...
  /**
   * 🆕 Utilitaire pour formater les erreurs pour l'affichage
   */
  formatError(error: TransformationError): string {
    switch (error.code) {
      ...
      case 409:
        return `Doublon détecté: ${error.message}`;
      case 404:
