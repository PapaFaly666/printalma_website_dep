...
interface AdminCreateVendorProductData {
  ...
  // Étape 2: Produit de base
  baseProductId?: number;
  selectedBaseProduct?: BaseProduct;
  
  // Étape 3: Design (V2 - support des deux modes)
  ...
  currentStep: number;
  lastSaved?: string;
  previewColorId?: number; // Pour la prévisualisation
  selectedMockupIds?: number[]; // Pour les mockups sélectionnés
}

const LOCALSTORAGE_KEY = 'admin_create_vendor_product_data';
const STEPS = [
  { id: 1, title: 'Vendeur', icon: User, description: 'Sélectionner le vendeur' },
  { id: 2, title: 'Produit', icon: Package, description: 'Choisir le produit de base' },
  { id: 3, title: 'Design', icon: Palette, description: 'Sélectionner le design' },
  { id: 4, title: 'Configuration', icon: Settings, description: 'Configurer le produit' },
  { id: 5, title: 'Aperçu', icon: Eye, description: 'Prévisualiser et finaliser' }
];

// Import des services
import designService, { Design as DesignType } from '../../services/designService';
import { vendorProductService } from '../../services/vendorProductService';

// Import du service de sauvegarde des transforms
import { saveDesignTransforms } from '../../services/designTransformsAPI';

// Composant de positionnement interactif pour l'admin copié de SellDesignPage.tsx
...
// Hook simplifié pour la sauvegarde localStorage dans l'admin
  ...
  React.useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        console.log('📥 Positions admin chargées depuis localStorage:', parsedStates);
      }
    } catch (error) {
      console.error('❌ Erreur chargement positions admin:', error);
    }
  }, [getStorageKey]);
  ...
  const saveToLocalStorage = React.useCallback((states: Record<number, { x: number; y: number; scale: number; rotation: number }>) => {
    ...
    saveTimeoutRef.current = setTimeout(() => {
      try {
        ...
        console.log('💾 Positions admin sauvegardées:', states);
      } catch (error) {
        console.error('❌ Erreur sauvegarde positions admin:', error);
      } finally {
        setIsSaving(false);
      }
    }, 300);
...
const AdminDesignPositioner: React.FC<AdminDesignPositionerProps> = ({
  ...
  // Récupérer les délimitations pour la couleur sélectionnée
  ...
  React.useEffect(() => {
    const transform = getTransform(0); // Délimitation principale
    ...
    if (pos && pos.width > 0 && pos.height > 0) {
      // Convertir la position relative de la délimitation vers des pourcentages du container global
      ...
      // Vérifier si la position a vraiment changé avant d'appeler onPositionChange
      if (
  ...
  return (
    <div className="relative w-full aspect-square bg-gray-50 rounded-lg overflow-hidden border border-gray-200">
      <div ref={containerRef} className="relative w-full h-full">
        ...
        {/* Design positionné - EXACTEMENT comme dans SellDesignPage.tsx */}
        {design && delimitations.map((delim: any, idx: number) => {
          ...
          // 🎯 CONTRAINTES EXACTES de SellDesignPage.tsx : Limiter le déplacement pour rester dans la délimitation
          ...
          return (
            <div
              ...
              tabIndex={0}
              title="Déplacez ou redimensionnez le design"
            >
              <div
              ...
              {/* Design avec contraintes appliquées */}
              <img
              ...
              {/* 4 poignées de resize exactement comme SellDesignPage */}
              {hoveredIdx === idx && [
        ...
        {/* Boutons de contrôle comme SellDesignPage */}
        <div className="absolute bottom-2 left-2 flex gap-2 z-20">
          <Button
            ...
            title="Réinitialiser les positions et tailles"
          >
        ...
        <div className="absolute bottom-2 right-2 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded">
          🎯 Glissez le design dans les zones bleues
        </div>
      </div>
    </div>
  );
...
export const AdminCreateVendorProductPage: React.FC = () => {
  ...
  // États pour la modal de sélection des vendeurs
  ...
  // Données du formulaire sauvegardées dans localStorage
  ...
  const fetchVendors = async () => {
    try {
      ...
      console.log('🔍 [DEBUG] Données vendeurs brutes:', data);
      if (data.vendors && data.vendors.length > 0) {
        console.log('🔍 [DEBUG] Premier vendeur:', data.vendors[0]);
        console.log('🔍 [DEBUG] Champs disponibles:', Object.keys(data.vendors[0]));
      }
  ...
  // Charger les designs du vendeur sélectionné
  const fetchVendorDesigns = async (vendorId: number) => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          console.log(`Aucun design trouvé pour le vendeur ${vendorId}`);
          setVendorDesigns([]);
          return;
        }
  ...
  useEffect(() => {
    ...
    // Si un vendeur est déjà sélectionné, charger ses designs
  ...
  // Fonction mémorisée pour éviter la boucle infinie
  ...
  const selectVendor = (vendor: VendorOption) => {
    ...
    toast.success(`Vendeur ${vendor.firstName} ${vendor.lastName} sélectionné`);
    nextStep();
  };
  ...
  const selectBaseProduct = (product: BaseProduct) => {
    ...
    updateFormData({
      ...
      // Initialiser les mockups sélectionnés (tous par défaut)
  ...
  const selectDesign = (design: Design) => {
    ...
    toast.success(`Design "${design.name}" sélectionné`);
    nextStep();
  };
  ...
  const createProduct = async () => {
    ...
    if (!formData.vendorId || !formData.baseProductId || !formData.productStructure) {
      toast.error('Données manquantes pour la création');
      return;
    }
    ...
    if (formData.designMode === 'new') {
      ...
      if (!newDesign?.name || !newDesign?.category || !newDesign?.imageBase64 || !newDesign?.price) {
        toast.error('Veuillez compléter toutes les informations du nouveau design (nom, catégorie, image, prix)');
        return;
      }
    }
    ...
    try {
      ...
      // 🔧 Récupérer les transforms depuis localStorage comme dans SellDesignPage.tsx
      ...
      if (designUrl) {
        ...
        if (savedTransforms) {
          try {
            ...
            // Récupérer les transforms de la délimitation principale (index 0)
            ...
            if (mainTransform) {
              ...
              console.log('📥 Transforms récupérés du localStorage:', localStorageTransforms);
            }
          } catch (e) {
      ...
      const basePayload = {
        ...
        finalImagesBase64: {} // Ajout requis
      };
      ...
      console.log('📤 Envoi données V2 avec transforms localStorage:', createData);
      ...
      // 🔧 Sauvegarder les transforms en base de données après la création comme dans SellDesignPage.tsx
      if (result.productId && designUrl) {
        ...
        try {
          ...
          console.log('✅ Transforms sauvegardés en base de données:', transformsPayload);
        } catch (transformError) {
          console.warn('⚠️ Erreur sauvegarde transforms (produit créé mais transforms non sauvegardés):', transformError);
        }
        ...
        if (formData.designMode === 'existing' && formData.designId) {
          ...
          try {
            ...
            console.log('✅ Position sauvegardée dans ProductDesignPosition:', positionPayload);
          } catch (positionError) {
            console.warn('⚠️ Erreur sauvegarde position:', positionError);
          }
        }
        ...
        if (designUrl) {
          ...
          console.log('🧹 localStorage nettoyé:', storageKey);
        }
      }
      
      // Gérer la réponse selon la documentation V2
      let successMessage = `Produit créé avec succès !`;
      if (result.newDesignCreated) {
        successMessage += ` Un nouveau design a été créé (ID: ${result.designId})`;
      }
      ...
      // Vider les données et rediriger
      clearData();
      navigate('/admin/vendor-products');
      
    } catch (err) {
      console.error('Erreur création produit V2:', err);
      
      // Gestion des erreurs spécifiques selon la documentation
      let errorMessage = 'Erreur lors de la création du produit';
      
      if (err.message.includes('400')) {
        if (err.message.includes('Vous ne pouvez pas fournir à la fois')) {
          errorMessage = 'Configuration de design invalide';
        } else if (err.message.includes('Vous devez fournir soit un designId')) {
          errorMessage = 'Veuillez sélectionner un design ou en créer un nouveau';
        } else {
          errorMessage = 'Données de création invalides';
        }
  ...
  // Gérer l'upload d'un nouveau design
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Choisissez pour quel vendeur vous voulez créer le produit
        </p>
      </div>

      {/* Vendeur sélectionné ou bouton de sélection */}
  ...
  // Rendu sélection produit
  const renderProductSelection = () => (
    <div className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-semibold text-gray-900">
          Sélectionner le produit de base
        </h3>
        <p className="text-sm text-gray-500 mt-1">
  ...
  const renderDesignSelection = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Choisissez un design existant de {formData.selectedVendor?.firstName} {formData.selectedVendor?.lastName} ou créez-en un nouveau
        </p>
      </div>
        ...
        <>
          ...
          {filteredDesigns.length === 0 && (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">Aucun design trouvé pour ce vendeur</p>
              <button
                onClick={() => updateFormData({ designMode: 'new' })}
  ...
  const renderConfiguration = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Échelle: {((formData.designPosition?.scale || 1) * 100).toFixed(0)}%
                </label>
                <input
                  type="range"
                  min="0.1"
          ...
          {/* Options avancées */}
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <h4 className="font-medium text-gray-900 mb-4">Options avancées</h4>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Statut forcé
        ...
        {/* Prévisualisation interactive à droite */}
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <h4 className="font-medium text-gray-900 mb-4">Prévisualisation</h4>
            
            {/* Sélection de la couleur pour prévisualisation */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Couleur pour prévisualisation
              </label>
              <div className="flex space-x-2">
            ...
            <div className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Couleurs sélectionnées:</span>
                <span className="font-medium">{formData.selectedColors?.length || 0}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Tailles sélectionnées:</span>
  ...
  const renderPreview = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Vérifiez les détails avant de créer le produit
        </p>
      </div>
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Mockups sélectionnés:</span>
  ...
  // Composant Modal de sélection des vendeurs
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden border border-gray-200">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Sélectionner un vendeur</h2>
            <p className="text-sm text-gray-500 mt-1">
              {filteredAndSortedVendors.length} vendeur{filteredAndSortedVendors.length > 1 ? 's' : ''} trouvé{filteredAndSortedVendors.length > 1 ? 's' : ''}
            </p>
          </div>
          <button
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-500">
              {filteredAndSortedVendors.length} vendeur{filteredAndSortedVendors.length > 1 ? 's' : ''} affiché{filteredAndSortedVendors.length > 1 ? 's' : ''}
            </p>
            <button
              onClick={() => setShowVendorModal(false)}
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-semibold tracking-tight text-gray-900">
                  Créer un produit pour vendeur
                </h1>
                <p className="text-sm text-gray-500">
                  Étape {formData.currentStep} sur {STEPS.length}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="text-xs text-gray-500">
                {formData.lastSaved && (
                  <span>Sauvegardé {new Date(formData.lastSaved).toLocaleTimeString()}</span>
                )}
              </div>
            </div>
          </div>
